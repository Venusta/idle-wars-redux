...
const BuildingConstruct = ({ buildingId, queuedLevel }: { buildingId: BuildingIdType, queuedLevel: number }) => {
  ...
  return (
    <div className={Style["building-grid-item"]}>
      <ConstructButton handleClick={() => dispatch(startBuildSomething({ townId, buildingId, queueBuildingId: BuildingId.Headquarters }))} text={`Level ${queuedLevel}`} />
    </div>
  );
};
...
export const Headquarters = (): JSX.Element => (
  <div className={Style.wrapper}>
    ...
    {baseBuildings[BuildingId.Headquarters].creates.map((id, index) => <BuildingRow key={`${id} + ${index}`} buildingId={id} />)}
  </div>
);
